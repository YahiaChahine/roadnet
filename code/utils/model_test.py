...
def _load_and_validate_intersection_index_map(dic_path, dic_traffic_env_conf):
    ...
    if stored_map != current_map:
        raise ValueError(
            "Mismatched intersection index mapping. Refusing to evaluate. "
    ...
    if len(stored_map) != dic_traffic_env_conf["NUM_AGENTS"]:
        raise ValueError(
            "Mismatched intersection index mapping size. Refusing to evaluate. "
...
def test(model_dir, cnt_round, run_cnt, _dic_traffic_env_conf):
    ...
    _load_and_validate_intersection_index_map(dic_path, dic_traffic_env_conf)
