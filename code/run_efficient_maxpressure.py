...
def parse_args():
    ...
    parser.add_argument("--dataset", type=str, default="jinan",
                        choices=get_supported_datasets("run_efficient_maxpressure.py"),
                        help="Dataset key. For xuancheng_mod flow files, use code/crop_x.py to generate valid one-hour files (xuancheng_YYYY_MM_DD_8to9.json).")
...
def main(in_args):
    ...
    dataset_conf = get_dataset_config("run_efficient_maxpressure.py", in_args.dataset)
    count = dataset_conf["run_counts"]
    traffic_file_list = dataset_conf["traffic_files"]
    template = dataset_conf["template"]
    roadnet_file = dataset_conf["roadnet_file"]
