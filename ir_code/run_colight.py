...
def _validate_colight_dependencies():
    missing = []
    if not os.path.exists("agent/dqn_agent.py"):
        missing.append("agent/dqn_agent.py")
    if not os.path.exists("model/model_tf/baseline/CoLight_model.py"):
        missing.append("model/model_tf/baseline/CoLight_model.py")
    if missing:
        print("CoLight dependencies are missing from this repository:")
        for m in missing:
            print(f" - {m}")
        print("Please restore these files or use a repo version that includes RL base/model code.")
        return False
    return True


def main():
    parser = argparse.ArgumentParser(description="Run CoLight training")
    ...
    if not _validate_colight_dependencies():
        sys.exit(2)

    from agent.baseline.CoLight_agent import CoLight

    # train.py simulate() expects a global Agent symbol
    train_module.Agent = CoLight
    ...
    avg_travel_time = evaluate_avg_travel_time(CoLight, args.config, args.output_dir, eval_duration, args.decision_interval, args.thread_num)
    metrics = {
        "output_dir": args.output_dir,
    ...
    print(f"Saved CoLight metrics: {args.output_dir}/metrics.json")
