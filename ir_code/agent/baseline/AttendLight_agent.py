...
from model.model_tf.baseline.AttendLight_model import AttendLightModel

class AttendLight(DQNAgent):
    name='AttendLight'
    def __init__(self, env, decision_interval, output_dir=None):
        ...
        """build model"""
        self.model = AttendLightModel(self.max_phase,env.env_config)
        self.target_model = AttendLightModel(self.max_phase,env.env_config)
    ...
    def observe(self, env, cal_reward=False, dyna={}):
        ...
        for agent_id in self.agent_list:
            ...
            if cal_reward:
                reward[agent_id] = -np.abs((np.sum(lane_queue_list[:12] *np.array(self.action_lane)) -
                        np.sum(lane_queue_list[12:] *np.array(self.action_lane)))) 
        if cal_reward:
            return state, reward
        else:
            return state
    ...
    #     value_loss_list = [];target_list = [];reward_list = []
    ...
    #         target_list.append(batch_target.max(axis=1).reshape(-1, 1))
    ...
    #     np.save('/'.join((self.cache_dir, 'batch_target.npy')),  np.mean(np.array(target_list)-np.array(reward_list),-2))
    #     np.save('/'.join((self.cache_dir, 'batch_reward.npy')), np.mean(np.array(reward_list),-2))
