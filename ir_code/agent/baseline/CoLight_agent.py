...
from model.model_tf.baseline.CoLight_model import CoLightModel
from agent.reward_func import regular_delay

class CoLight(DQNAgent):
    name='CoLight'
    def __init__(self, env, decision_interval, output_dir=None):
        ...
        self.nei_feature_list = ['up_lane_dens', 'up_lane_speed', 'up_lane_num', 'down_lane_dens', 'down_lane_speed', 'down_lane_num',\
                                 'cur_phase']
        
        """build model"""
        self.model = CoLightModel(self.max_phase)
        self.target_model = CoLightModel(self.max_phase)
    ...
    def observe(self, env, cal_reward=False, dyna={}):
        ...
        for agent_id in self.agent_list:
            ...
            #### 需添加额外属性
            ##  ['flow_index', 'total_output', 'total_pressure', 'total_queue_length',
            ##   'total_delay', 'total_approx_delay' ]
            # agent_lane_queue = [lane_queue.get(l,0) for l in agent_lane]
            state[agent_id] = {
                'up_lane_dens':[lane_density.get(l, 0) for l in agent_lane[:12]],
                'up_lane_speed':[lane_avg_speed.get(l, -1) for l in agent_lane[:12]],
                'up_lane_num': [lane_veh_num.get(l, 0) for l in agent_lane[:12]],
                'down_lane_dens': [lane_density.get(l, 0) for l in agent_lane[12:]],
                'down_lane_speed': [lane_avg_speed.get(l, -1) for l in agent_lane[12:]],
                'down_lane_num':[lane_veh_num.get(l, 0) for l in agent_lane[12:]],
            ...
            if cal_reward:
              inlane_queue = [lane_queue.get(l, 0) for l in agent_lane[:12]]
              action_lane = env.env_config.action_lane[:len(inlane_queue)]
              reward[agent_id] = -np.mean(np.array(action_lane) * np.array(inlane_queue))
        if cal_reward:
            return state, reward
        else:
            return state
    ...
    def _create_action_inputs(self, state):
        batch_state = super()._create_action_inputs(state)
        batch_nei_state = {'nei_' + f:[] for f in self.nei_feature_list}
        batch_nei_state['nei_inter_mask'] = []
        # 各交叉口信息
        for a in self.agent_list:
            nei_agent_id = self.inter_adj[a][0]
            # nei_road_id = self.inter_adj[a][1]
            for i in range(4):
                try:
                    agent_id = nei_agent_id[i]
                    neigh_state = state[agent_id]
                    for f in self.nei_feature_list:
                        batch_nei_state['nei_' + f].append(neigh_state[f])
                    batch_nei_state['nei_inter_mask'].append(1)
                except:
                    for f in self.nei_feature_list:
                        batch_nei_state['nei_' + f].append(state[a][f])
                    batch_nei_state['nei_inter_mask'].append(0)
        ## 邻交叉口信息
        for f in self.nei_feature_list:
            batch_nei_state['nei_' + f] = np.reshape(np.array(batch_nei_state['nei_'+f]), (len(self.agent_list),4,-1))
        batch_nei_state['nei_inter_mask'] = np.reshape(np.array(batch_nei_state['nei_inter_mask']), (len(self.agent_list),4,-1))
        batch_state.update(batch_nei_state)
        return batch_state
    ...
    def _create_buffer(self):
        ...
        batch_next_state={f:[] for f in self.feature_list}
        for a in self.agent_list:
            batch_action.extend(self.memory[a]['action'])
            batch_reward.extend(self.memory[a]['reward'])
            for f in self.feature_list:
                batch_state[f].extend(s[f] for s in self.memory[a]['state'])
                batch_next_state[f].extend(ns[f] for ns in self.memory[a]['next_state'])
        for f in self.feature_list:
        ...
        print(f'{datetime_utc8()} Buffer size: ', len(batch_reward))
        batch_reward = self._reward_normalize(batch_reward)
        ##### 邻交叉口信息
        batch_nei_state = {}
        batch_nei_next_state = {}
        for f in self.nei_feature_list:
            batch_nei_state['nei_' + f] = []
            batch_nei_next_state['nei_' + f] = []
        batch_nei_state['nei_inter_mask']=[]
        batch_nei_next_state['nei_inter_mask']=[]
        for a in self.agent_list:       
            nei_agent_id = self.inter_adj[a][0]
            for i in range(4):
                try:
                    nei_agent_id = nei_agent_id[i]
                    for f in self.nei_feature_list:
                        batch_nei_state['nei_'+f].append(s[f] for s in self.memory[nei_agent_id]['state'])
                        batch_nei_next_state['nei_'+f].extend(s[f] for ns in self.memory[nei_agent_id]['next_state'])
                    batch_nei_state['nei_inter_mask'].extend(np.ones([len(self.memory['intersection_1_1']['state']),1]))
                    batch_nei_next_state['nei_inter_mask'].extend(np.ones([len(self.memory['intersection_1_1']['state']),1]))
                except:
                    for f in self.nei_feature_list:
                        batch_nei_state['nei_'+f].extend(s[f] for s in self.memory[a]['state'])
                        batch_nei_next_state['nei_'+f].extend(ns[f] for ns in self.memory[a]['next_state'])
                    batch_nei_state['nei_inter_mask'].extend(np.zeros([len(self.memory['intersection_1_1']['state']),1]))
                    batch_nei_next_state['nei_inter_mask'].extend(np.zeros([len(self.memory['intersection_1_1']['state']),1]))
        buffer_len = len(self.memory) * len(self.memory['intersection_1_1']['state'])
        for f in self.nei_feature_list:
            batch_nei_state['nei_' + f] = np.array(batch_nei_state['nei_'+f])
            batch_nei_next_state['nei_' + f] = np.array(batch_nei_next_state['nei_'+f])
        for f in self.nei_feature_list+['inter_mask']:
            batch_nei_state['nei_' + f] = np.reshape(batch_nei_state['nei_'+f], (buffer_len,4,-1))
            batch_nei_next_state['nei_' + f] = np.reshape(batch_nei_next_state['nei_'+f], (buffer_len,4,-1))
        batch_state.update(batch_nei_state)
        batch_next_state.update(batch_nei_next_state)
    ...
    def init_memory(self):
        self.memory = {a:{item:[] for item in['state','action','reward','next_state'] } for a in self.agent_list}


    def memorize(self, last_states, actions, rewards, states):
        for a in self.agent_list:
            self.memory[a]['state'].extend([last_states[a]])
            self.memory[a]['action'].extend([actions[a]])
            self.memory[a]['reward'].extend([rewards[a]])
            self.memory[a]['next_state'].extend([states[a]])


    def _load_memory(self, new_memory):
        for a in self.agent_list:
            self.memory[a]['state'].extend(new_memory[a]['state'])
            self.memory[a]['action'].extend(new_memory[a]['action'])
            self.memory[a]['reward'].extend(new_memory[a]['reward'])
            self.memory[a]['next_state'].extend(new_memory[a]['next_state'])
